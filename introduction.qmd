...
* Reports (including dashboards). You are likely to produce these using either Quarto or RMarkdown. In this book we'll focus on Quarto, but the same ideas apply to RMarkdown too.

* Interactive Shiny apps. These are the bread and butter of most data scientists. Like reports, they're aimed at humans, but instead of being a static document that's only controlled by the data scientist, they're an interactive collaboration between the data scientist and the end-user.

* Plumber APIs (including models). Sometimes you don't need to communicate to other humans inside your organisation, but other computers. This is where an API comes in: you provide a API that's a black box from the outside but runs R code inside. A common use case of APIs for data scientists is to provider access to models: the caller supplies some data and the model returns a prediction or classification.

* Websites (including books and blogs). Sometimes you need more than one quarto file, you need multiple quarto files that live in one directory and are cross-linked. You can think of this as a book if it's something that you'd tend to read through in linear order, a blog if it's primarily organised in chronological order, or otherwise a website. I think every team should have at least one website, a quarto site where you define your team's recommended practices.
