...
There are always at least two steps when putting a job into production:

* From your development environment, you deploy **deploy** your code to the production environment. This captures all dependencies that your code needs to run and transmits them along with your code to the production environment.

* The production environment then **executes** the job, either on a schedule or on demand.
...
As you'll learn, we also recommend an additional **test** step which is in run just prior to deployment. If the tests fail, the deployment fails, ensuring that you never deploy code with known bugs (unfortunately, there's no way to protect against unknown bugs!).
