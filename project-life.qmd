...
In this chapter, you'll learn about how to live the project-oriented life, how you organise your file and how you share them with others.
...
-   You should only ever use relative paths in your project[^project-life-1].
    If you share your project with others, you can't expect your project to live to in the same place, so if you've used absolute paths, they are unlikely to work.
...
While your code is going to fetch data and possibly save it locally, probably won't want to commit it to your repo (that's a recipe for your repo exploding in size).
You should make sure to that sort of data in your `.gitignore` so that you never accidentally commit it.
...
Whenever you have multiple copies of something, you need to carefully think about which think is going to be the "source of truth" (i.e. if they're different, which one do you believe?).
...
![A collaborative development workflow where multiple team members interact with a central git repository, and that repository goes from git to production.](diagrams/project-life.svg){#fig-single-source fig-alt="A flowchart diagram showing a Git workflow with five boxes connected by arrows. The boxes are labeled \"You\", \"git\", \"Production\", \"Colleague A\", and \"Colleague B\". Bidirectional arrows connect \"You\", \"Colleage A\", and \"Colleague B\" with \"git\", a one-direcitonal arrow goes from \"git\" to \"Production\" (dashed line). T"}

We recommend this workflow, because while Git takes some time to learn, it provides a principle way to maintain and share code, tracking the provenance of every change.
...
If you haven't used Git before, I'd recommend that put this book down now, and read [Happy Git and GitHub for the useR](https://happygitwithr.com/).
...
Throughout the rest of the book, we'll talk about GitHub exclusively, but this is really just a short hand for saying your "Git-hosting platform".
...
You're unlikely to get to choose so you'll need to adapt to what you're given; but this is generally not too hard since the modern platforms are all provide pretty similar tools.
...
But GitHub is also great when you're a solo data science or learning data science.
...
If you're just starting out in your career, GitHub is also a great place to build up portfolio of data science projects that you can talk about when interviewing/some of the production jobs you've created for your own life.
If you're in that position, I'd highly recommend watching [GitHub: How To Tell Your Professional Story](https://www.youtube.com/watch?v=R-bPjavai5A), by Abigal Haddad.
...
It's totally fine to start with click-to-deploy to learn the basics, but over time you want to transition to a fully automate solution.
...
As we'll see, you can configure a branch to deploy into your staging environment so that you can experiment with confidence without having to worry that stakeholders will see your mistake.
