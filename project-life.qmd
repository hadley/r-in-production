...
-   `artifact/`: the directory where the app/api/report/script lives.
    If you have multiple deployable artefacts in the same project, or a large number of R scripts, you can use sub-directories of `artifact/`.
    (The only time you won't have an `artifact/` directory is if when you're deploying a package, in which case the project root itself will be deployed.)
