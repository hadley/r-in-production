...
-   `artifact/`: the directory where the app/api/report and its dependencies lives.
    If you have multiple deployable artefacts in the same project, these will live in sub-directories.

-   `README.md`: overall project level documentation.

-   A lockfile that specifies the dependencies needed to execute your artefact.

Most projects will also grow to include two additional projects:

-   `R/`: standalone functions that are used in multiple places in your code.

-   `tests/testthat/`: tests for your standalone functions.

Additionally you might also have:

-   `data/`: small supplementary datasets and sample data used for testing.

-   `src/`: in the rare case that you need compiled C code.

For naming the files that live inside these directories, we recommend that you follow the advice in the [tidyverse style guide](https://style.tidyverse.org/files.html#names).
...
That means absolute paths won't work and you need to use a relative path[^project-life-1].
...
-   **Project-relative** paths start at the project root, and can be created with `here::here()`.
    You should not need to use these too frequently because our tools will automatically load files from the most commonly used project directories.
    For example `pkgload::load_all()` automatically sources all files in `R/` and lazily loads all datasets in `data/`.
...
The code that you deploy should come from your central Git repo, not your local computer.
