...
In this chapter, you'll learn about our recommend practices for organising your production-bound projects.
This advice helps you projects for yourself, but is particularly important when you're collaborating with others.
...
But the most important thing is to develop consistent principles with the people you work with.
So if you want to deviate from what we describe here, just make sure to write it down!

So far we have three important principles:
...
This directory should include contain all the files needed to run the project and all files created by the project; your code should not read or write any files outside of the project directory.
...
Additionally, when you share your project with other people, you can't expect it to live in the same place, so absolute paths won't work.
Instead, always use a relative paths[^project-life-1].
Relative paths are relative to the **working directory**, which should always be the project directory[^project-life-2]
.

[^project-life-1]: i.e. never start the path with `/`, `C:/`, or `~.`

[^project-life-2]: In other words, don't change the working directory either by calling `setwd()` or clicking some button in your IDE.
...
You can also work around this problem by using `..` in the path.
We don't recommend this because that pattern signals that you are reaching outside of the working directory, something you generally want to avoid.

### Dependencies

A literal reading of the rule to only reading and writing inside your project directory imply that you can't use packages!
...
We'll go into these in much detail later in the book, but it's good to be aware of them now:

-   **R**: despite needing R to run your code, you'll never include it in your project.
    Unfortunately there's currently no standard way to record which version of R your package requires currently, but it's something we're working on.
...
-   **Secrets**: depending on how your authentication system works, you project may need secrets like passwords or API keys in order to run.
    ...
    We'll talk about alternativvs in ...

-   **Data**: by and large, you project will not include data inside of it, it will instead be retrieved on demand.
...
We recommend that you follow the advice in the tidyverse style guide on [naming files](https://style.tidyverse.org/files.html#names).
