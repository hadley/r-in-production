...
Before we dive into the biggest challenges of working on another machine, I wanted to quickly cover a bunch of annoying paper cuts. None of these are particularly hard to work around, but they're likely to be confusing the first time you encounter them and they continue to be a minor nuisance for long into in your production journey.
...
Absolute paths look a little different depending on your operating system: on Windows they start with a drive letter (e.g., `C:`) or two backslashes (e.g., `\\servername`) but on Mac/Linux they start with a slash “/” (e.g., `/users/hadley`). Fortunately this should rarely be a problem for production scripts since they aren't going to work anyway because you can't assume that your production environment has exactly the same directory configuration as your development environment.
