...
The final path issue is that paths on Linux are generally case-sensitive, and paths on Windows are not. If you're ever scratching your head wondering why your production script can't find a file that's very obviously there, start by double-checking that the case!
...
On your Windows machine you can configure git to automatically convert CRLF to LF:

```
git config --global core.autocrlf true
```

GitHub provides some [nice documentation](https://docs.github.com/en/get-started/getting-started-with-git/configuring-git-to-handle-line-endings) that gives more details if you're interested.
...
On Mac and Windows, it's pretty straightforward to know how to install R: you go to <https://www.r-project.org> and find the appropriate installer. How do you install R on Linux? There are two ways:

1.  Using your system package manager, e.g. with `sudo yum install R`.
2.  Using [rig](https://github.com/r-lib/rig).

We recommend using rig because it makes it easy to install any version of R, you can install multiple versions and switch between them, and it automatically sets you up for R package installation success by installing pak and setting up the right CRAN mirrors. (More on that in @sec-package-install). On Linux, it installs R built at <https://github.com/rstudio/r-builds>, which also includes backported fixes for any major CVEs.

The main downside to using rig is that it's a little bit more work to install it by [following the instructions](https://github.com/r-lib/rig?tab=readme-ov-file#installing-rig-on-linux-) on its website. But once you've got it, it's really easy to install new versions of R:
...
Moreso than differences in graphic devices, what might make your output look different between local and production environment is the fonts available during rendering of graphics. This is something that will inevitably crop up, even if you use ragg, since the default fonts ("sans", "serif", "mono", and "symbol") are mapped to different fonts depending on your operating system. If you decide to be explicit about the fonts in use and provide the full name of the fonts you use, you run into the problem that this font might not be available on the production environment. While most users are comfortable installing new fonts on their own machine, installing fonts on a Linux server is a different challenge entirely.

There is no perfect solution at the moment. Asking the system administrator to make sure that certain fonts are available on the server is of course possible, but also far from agile. If you use ragg or svglite you can make use of systemfonts ability to register fonts not installed on the system and provide the font files as part of the deployment. We are planning to improve on this process in the future so that needed fonts can be declared in your code and automatically fetched if missing, but we are not there yet.
