...
Before we dive into the biggest challenges of working on another machine, I wanted to quickly cover a bunch of annoying paper cuts. None of these are particularly hard to work around, but they're likely to confusing the first time you encounter them and they continue to be a minor nuisance for long in your production journey.
...
The final path issue that you might run into is that paths on Linux are generally case-sensitive, and paths on Windows are not. If you're ever scratching your head wondering why your production script can't find a file that's very obviously there, start by double-checking the case! (The most common way that this affects me is that I refer to `foo.r` but I have actually created `foo.R`.)
...
The main downside to using rig is that installing it the first time a bit more work, and you'll need to the [follow the instructions](https://github.com/r-lib/rig?tab=readme-ov-file#installing-rig-on-linux-) on its website. But once you've got it, it's really easy to install new versions of R:
...
If you are producing PNG graphics, be aware that the underlying implementation of the `png()` graphics device varies from platform to platform. That means the rendering of your plots is going to be a little different when rendering them on Linux (but you may find they look better than on Windows, where the default png device is not so good). Most of the time this is isn't too important, but if you really care about the details of your plots it's worth knowing how to do better.
